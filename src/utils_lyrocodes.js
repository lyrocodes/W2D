...
const updater = {
  ...
  async fetchLatestVersion() {
    ...
    state.logger.error("[ W2D ] Tag name wasn't in result");
    return null;
  },
  ...
  async downloadLatestVersion(defaultExeName, name) {
    return requests.downloadFile(name, `https://github.com/lyrocodes/W2D/releases/latest/download/${defaultExeName}`);
  },

  async downloadSignature(defaultExeName) {
    const signature = await requests.fetchBuffer(`https://github.com/lyrocodes/W2D/releases/latest/download/${defaultExeName}.sig`);
    if ('error' in signature) {
      console.log("[ W2D ] Couldn't fetch the signature of the update.");
      return false;
    }
    return signature;
  },
  ...
  async update() {
    ...
    if (!defaultExeName) {
      console.log(`[ W2D ] Auto-update is not supported on this platform: ${os.platform()}`);
      return false;
    }
    ...
    if (!downloadStatus) {
      console.log('[ W2D ] Download failed! Skipping update.');
      return false;
    }
    ...
    if (signature && !this.validateSignature(signature.result, currExeName)) {
      console.log("[ W2D ] Couldn't verify the signature of the updated binary, reverting back. Please update manually.");
      this.revertChanges();
      return false;
    }
  ...
  async run(currVer) {
    if (process.argv.some(arg => ['--skip-update', '-su'].includes(arg))) {
      console.log('[ W2D ] Skipping update due to command line argument.');
      return;
    }

    if (this.isNode) {
      console.log('[ W2D ] Running script with node. Skipping auto-update.');
      return;
    }
    ...
    if (newVer === null) {
      console.log('[ W2D ] Something went wrong with auto-update.');
      return;
    }
    ...
    if (prompt !== 'y') {
      console.log('[ W2D ] Skipping update.');
      return;
    }

    console.log('[ W2D ] Please wait as the bot downloads the new version.');
    const exeName = await updater.update();
    if (exeName) {
      await ui.input(`[ W2D ] Updated WA2DC. Hit enter to exit and run ${this.currentExeName}.`);
      process.exit();
    }
  },
...
const sqliteToJson = {
  ...
  async downloadAndVerify() {
    ...
    if (exeName == '') {
      console.log(`[ W2D ] Automatic conversion of database is not supported on this platform and arch ${os.platform()}/${process.arch}. Please convert database manually`);
      return false;
    }

    const downloadStatus = await this.downloadLatestVersion(exeName);
    if (!downloadStatus) {
      console.log('[ W2D ] Download failed! Please convert database manually.');
      return false;
    }

    const signature = await this.downloadSignature(exeName);
    if (signature && !updater.validateSignature(signature.result, exeName)) {
      console.log("[ W2D ] Couldn't verify the signature of the database converter. Please convert database manually");
  ...
  async repairChannels() {
    ...
    if (!categoryExists) {
      state.settings.Categories[0] = (
        await guild.channels.create('whatsapp by lyrocodes', {
          type: 'GUILD_CATEGORY',
        })
      ).id;
    }
    ...
    if (!controlExists) {
      state.settings.ControlChannelID = (await this.createChannel('w2d-control-room')).id;
    }

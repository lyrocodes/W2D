...
const setup = {
  async setupDiscordChannels(token) {
    return new Promise((resolve) => {
      ...
      client.once('ready', () => {
        console.log(`[ W2D ] Invite the bot using the following link: https://discordapp.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=536879120`);
      });
      client.once('guildCreate', async (guild) => {
        const category = await guild.channels.create('W2D-lyrocodes', {
          type: 'GUILD_CATEGORY',
        });
  ...
  async firstRun() {
    ...
    console.log('[ W2D ] It seems like this is your first run.');
    if (process.env.WA2DC_TOKEN === "CHANGE_THIS_TOKEN") {
      console.log("[ W2D ] Please set WA2DC_TOKEN environment variable.");
      process.exit();
    }
    ...
    settings.Token = process.env.WA2DC_TOKEN || await input('[ W2D ] Please enter your bot token and press enter : ');

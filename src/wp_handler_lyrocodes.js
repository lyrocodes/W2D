...
const connectToWhatsApp = async (retry = 1) => {
    ...
    client.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (retry <= 3) {
                await controlChannel.send(`[ W2D ] WhatsApp connection failed! Trying to reconnect! Retry #${retry}`);
                await connectToWhatsApp(retry + 1);
            } else if (retry <= 5) {
                const delay = (retry - 3) * 10;
                await controlChannel.send(`[ W2D ]  WhatsApp connection failed! Waiting ${delay} seconds before trying to reconnect! Retry #${retry}.`);
                await new Promise((resolve) => { setTimeout(resolve, delay * 1000); });
                await connectToWhatsApp(retry + 1);
            } else {
                await controlChannel.send('[ W2D ]  Connection failed 5 times. Please rescan the QR code.');
            ...
            retry = 1;
            await controlChannel.send('[ W2D ]  WhatsApp connection successfully opened!');
        }
    });
    ...
    client.ev.on('discordMessage', async ({ jid, message }) => {
        ...
        if (message.reference) {
            ...
            if (options.quoted == null) {
                message.channel.send("[ W2D ] Couldn't find the message quoted. You can only reply to last 500 messages. Sending the message without the quoted message.");
            }
        }

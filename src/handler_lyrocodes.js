...
client.on('whatsappMessage', async (message) => {
  ...
  if (message.file) {
    if (message.file.largeFile && state.settings.LocalDownloads) {
      ...
      msgContent += "[ W2D ] Attention: Received a file, but it's over 8MB. Check WhatsApp on your phone or enable local downloads.";
    } else {
...
client.on('whatsappReaction', async (reaction) => {
  ...
  await message.react(reaction.text).catch(async err => {
    if (err.code === 10014) {
      await channel.send(`[W2D] Unknown emoji reaction (${reaction.text}) received. Check WhatsApp app to see it.`);
    }
  });
});
...
client.on('whatsappCall', async ({ call, jid }) => {
  ...
  switch (call.status) {
    case 'offer':
      content = `[ W2D ] ${name} is ${callType} calling you! Check your phone to respond.`
      break;
    case 'timeout':
      content = `[ W2D ] Missed a ${callType} call from ${name}!`
      break;
  }
...
const commands = {
  ...
  async pairwithcode(_message, params) {
    if (params.length !== 1) {
      await controlChannel.send('[ W2D ] Please enter your number. Usage: `pairWithCode <number>`. Don\'t use "+" or any other special characters.');
      return;
    }

    const code = await state.waClient.requestPairingCode(params[0]);
    await controlChannel.send(`[ W2D ] Your pairing code is: ${code}`);
  },
  async start(_message, params) {
    if (!params.length) {
      await controlChannel.send('[ W2D ] Please enter a phone number or name. Usage: `start <number with country code or name>`.');
      return;
    }
    ...
    if (!jid) {
      await controlChannel.send(`[ W2D ] Couldn't find \`${params.join(' ')}\`.`);
      return;
    }
  ...
  async addtowhitelist(message, params) {
    ...
    if (params.length !== 1 || !channelID) {
      await controlChannel.send('[ W2D ] Please enter a valid channel name. Usage: `addToWhitelist #<target channel>`.');
      return;
    }

    const jid = utils.discord.channelIdToJid(channelID);
    if (!jid) {
      await controlChannel.send("[ W2D ] Couldn't find a chat with the given channel.");
      return;
    }

    state.settings.Whitelist.push(jid);
    await controlChannel.send('[ W2D ] Added to the whitelist!');
  },
  async removefromwhitelist(message, params) {
    const channelID = /<#(\d*)>/.exec(message)?.[1];
    if (params.length !== 1 || !channelID) {
      await controlChannel.send('[ W2D ] Please enter a valid channel name. Usage: `removeFromWhitelist #<target channel>`.');
      return;
    }
  ...
  async setdcprefix(message, params) {
    if (params.length !== 0) {
      ...
      await controlChannel.send(`[ W2D ] Discord prefix is set to ${prefix}!`);
    } else {
      state.settings.DiscordPrefixText = null;
      await controlChannel.send('[ W2D ] Discord prefix is set to your discord username!');
    }
  },
  async enabledcprefix() {
    state.settings.DiscordPrefix = true;
    await controlChannel.send('[ W2D ] Discord username prefix enabled!');
  },
  async disabledcprefix() {
    state.settings.DiscordPrefix = false;
    await controlChannel.send('[ W2D ] Discord username prefix disabled!');
  },
  async enablewaprefix() {
    state.settings.WAGroupPrefix = true;
    await controlChannel.send('[ W2D ] WhatsApp name prefix enabled!');
  },
  async disablewaprefix() {
    state.settings.WAGroupPrefix = false;
    await controlChannel.send('[ W2D ] WhatsApp name prefix disabled!');
  },
  async enablewaupload() {
    state.settings.UploadAttachments = true;
    await controlChannel.send('[ W2D ] Enabled uploading files to WhatsApp!');
  },
  async disablewaupload() {
    state.settings.UploadAttachments = false;
    await controlChannel.send('[ W2D ] Disabled uploading files to WhatsApp!');
  },
  async help() {
    await controlChannel.send('[ W2D ] [Click](https://discord.gg/xxrxwTu9F4) to Join Our Discord Server and check the <#1227943712013680692> channel for commands documentation');
  },
  async about() {
    await controlChannel.send('You Are Using W2D Source Code by [lyrocodes](https://github.com/lyrocodes/W2D) \n Click to join Official LyroCodes Server \n Please Dont Remove Credits');
  },
  async resync() {
    ...
    await controlChannel.send('[ W2D ] Re-synced!');
  },
  async enablelocaldownloads() {
    state.settings.LocalDownloads = true;
    await controlChannel.send(`[ W2D ] Enabled local downloads. You can now download files larger than 8MB.`);
  },
  async disablelocaldownloads() {
    state.settings.LocalDownloads = false;
    await controlChannel.send(`[ W2D ] Disabled local downloads. You won't be able to download files larger than 8MB.`);
  },
  async getdownloadmessage() {
    await controlChannel.send(`[ W2D ] Download message format is set to "${state.settings.LocalDownloadMessage}"`);
  },
  async setdownloadmessage(message) {
    state.settings.LocalDownloadMessage = message.content.split(' ').slice(1).join(' ');
    await controlChannel.send(`[ W2D ] Set download message format to "${state.settings.LocalDownloadMessage}"`);
  },
  async getdownloaddir() {
    await controlChannel.send(`[ W2D ] Download path is set to "${state.settings.DownloadDir}"`);
  },
  async setdownloaddir(message) {
    state.settings.DownloadDir = message.content.split(' ').slice(1).join(' ');
    await controlChannel.send(`[ W2D ] Set download path to "${state.settings.DownloadDir}"`);
  },
  async enablepublishing() {
    state.settings.Publish = true;
    await controlChannel.send(`[ W2D ] Enabled publishing messages sent to news channels.`);
  },
  async disablepublishing() {
    state.settings.Publish = false;
    await controlChannel.send(`[ W2D ] Disabled publishing messages sent to news channels.`);
  },
  async enablechangenotifications() {
    state.settings.ChangeNotifications = true;
    await controlChannel.send(`[ W2D ] Enabled profile picture change and status update notifications.`);
  },
  async disablechangenotifications() {
    state.settings.ChangeNotifications = false;
    await controlChannel.send(`[ W2D ] Disabled profile picture change and status update notifications.`);
  },
  async autosaveinterval(_message, params) {
    if (params.length !== 1) {
      await controlChannel.send("[ W2D ] Usage: autoSaveInterval <seconds>\nExample: autoSaveInterval 60");
      return;
    }
    state.settings.autoSaveInterval = +params[0];
    await controlChannel.send(`[ W2D ] Changed auto save interval to ${params[0]}.`);
  },
  async lastmessagestorage(_message, params) {
    if (params.length !== 1) {
      await controlChannel.send("[ W2D ] Usage: lastMessageStorage <size>\nExample: lastMessageStorage 1000");
      return;
    }
    state.settings.lastMessageStorage = +params[0];
    await controlChannel.send(`[ W2D ] Changed last message storage size to ${params[0]}.`);
  },
  async oneway(_message, params) {
    if (params.length !== 1) {
      await controlChannel.send("[ W2D ] Usage: oneWay <discord|whatsapp|disabled>\nExample: oneWay whatsapp");
      return;
    }
    
    if (params[0] === "disabled") {
      state.settings.oneWay = 0b11;
      await controlChannel.send(`[ W2D ] Two way communication is enabled.`);
    } else if (params[0] === "whatsapp") {
      state.settings.oneWay = 0b10;
      await controlChannel.send(`[ W2D ] Messages will be only sent to WhatsApp.`);
    } else if (params[0] === "discord") {
      state.settings.oneWay = 0b01;
      await controlChannel.send(`[ W2D ] Messages will be only sent to Discord.`);
    } else {
      await controlChannel.send("[ W2D ] Usage: oneWay <discord|whatsapp|disabled>\nExample: oneWay whatsapp");
    }
  },
  async unknownCommand(message) {
    await controlChannel.send(`[ W2D ] Unknown command: \`${message.content}\`\nType \`help\` to see available commands`);
  },
};
...
client.on('messageUpdate', async (_, message) => {
  ...
  if (messageId == null) {
    await message.channel.send("[ W2D ] Couldn't edit the message. You can only edit the last 500 messages.");
    return;
  }
...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  if (messageId == null) {
    await reaction.message.channel.send("[ W2D ] Couldn't send the reaction. You can only react to last 500 messages.");
    return;
  }
...
client.on('messageReactionRemove', async (reaction, user) => {
  ...
  const messageId = state.lastMessages[reaction.message.id];
  if (messageId == null) {
    await reaction.message.channel.send("[ W2D ] Couldn't remove the reaction. You can only react to last 500 messages.");
    return;
  }
  if (user.id === state.dcClient.user.id) {
    return;
